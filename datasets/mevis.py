...
class MEVISDataset(Dataset):
    ...
    def __init__(self, args, img_folder: Path, ann_file: Path, transforms, return_masks: bool,
        ...
        self.mode = "train" if 'train' in str(self.img_folder) else 'val'
...
def build(image_set, args):
    ...
    PATHS = {
        ...
        "val": (root / "valid_u", root / "valid_u" / "meta_expressions.json"),  # not used actually
    }
    img_folder, ann_file = PATHS[image_set]
    dataset = MEVISDataset(args, img_folder, ann_file, transforms=make_coco_transforms(image_set, max_size=args.max_size), return_masks=args.masks,
...
if __name__ == '__main__':
    ...
    dataset = MEVISDataset(None, img_folder, ann_file, transforms=T.ToTensor(), return_masks=True, num_frames=5, max_skip=3)
