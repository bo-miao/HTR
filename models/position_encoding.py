...
class PositionEmbeddingSine2D(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        assert mask is not None
        not_mask = ~mask  # inverse pad mask.
        # pos on valid regions, false region will not increase the coord.
...
if __name__ == "__main__":
    m = PositionEmbeddingSine1D(256, normalize=True)
    d_i = torch.rand(1,6,256)  # bxlxc
    d_m = torch.zeros(1,6).bool()
    d = NestedTensor(d_i, d_m)
    print(m(d).shape)  # 1x256x6