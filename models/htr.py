...
from .segmentation import VisionLanguageFusionModule, CrossModalFPNDecoder
from transformers import BertTokenizer, BertModel, RobertaModel, RobertaTokenizerFast, T5Model, T5TokenizerFast
import copy
from einops import rearrange, repeat
import warnings
warnings.filterwarnings("ignore")
from models.vos.propagator import VOSHead
...
class HTRFormer(nn.Module):
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            ...
            num_queries: number of object queries, ie detection slot. This is the maximal number of objects
                         model can detect in a video. For ytvos, we recommend 5 queries for each frame.
        ...
        self.num_frames = num_frames  # clip frame
        ...
        if num_feature_levels > 1:
            ...
            for idx_ in range(num_backbone_outs):
                in_channels = backbone.num_channels[-3:][idx_]  # 192 -- 384 -- 768
            ...
            # only convert the last layer to hidden dim.
    ...
    def build_controller(self):
        ...
        self.in_channels = self.mask_dim  # 256
        self.dynamic_mask_channels = self.dynamic_mask_channels  # 8
    ...
    def forward(self, samples: NestedTensor, captions, targets, memory_visual=None, pad_div=16, external_mask=False):
        ...
        """
        if memory_visual is None:
            # Backbone
            if not isinstance(samples, NestedTensor):
                samples = nested_tensor_from_videos_list(samples, 1 if self.training else pad_div)  # eval cpk pred 16 masks for 1 token, need pad.
            features, visual_pos = self.backbone(samples)  # pos embed add another channel pos
        else:
            # for visual memory share
            samples, features, visual_pos = memory_visual

        b = len(captions)
        t = visual_pos[0].shape[0] // b

        ############## only need visual feature for inference with external masks (for collaboration with other models).
        if not self.training and external_mask:
            out = {}
            out["features"] = [x.tensors for x in features]  # [bt c h w]
            out["images"] = rearrange(samples.tensors, '(b t) c h w -> b t c h w', t=t)
            return out
        ####################################################################################

        if 'valid_indices' in targets[0]:
            if self.training:
                valid_indices = targets[0]['valid_indices']
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-multi_scale_level:], visual_pos[-multi_scale_level:])):
            ...
            src_proj_l = self.fusion_module(visual=src_proj_l,  # visual
        ...
        if self.num_feature_levels > len(srcs):
            _len_srcs = self.num_feature_levels - 1  # fpn level
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                src = self.fusion_module(visual=src,  # visual
                                         text=text_word_features,
                                         text_key_padding_mask=text_word_masks,
                                         text_pos=text_pos,
                                         visual_pos=None
        ...
        # Segmentation
        mask_features = self.pixel_decoder(features, text_features, visual_pos, memory, nf=t) # [batch_size*time, c, out_h, out_w]
        mask_features = rearrange(mask_features, '(b t) c h w -> b t c h w', b=b, t=t)
        del memory, text_features, visual_pos, srcs, masks, poses, text_word_features
    ...
    def mask_propagation(self, feat, mask, image, frame_memory, frame_query, sigmoid=True):
        ...
        query_feat = [x[:, frame_query].flatten(0,1) for x in feat]  # [bt c h w]
        if sigmoid:
            mask = mask.sigmoid()
        mem_mask = mask[:, frame_memory].unsqueeze(2)  # b t 1 h w
        mem_image = image[:, frame_memory]  # b t c h w
        query_image = image[:, frame_query]  # b t c h w
        ...
        assert len(frame_query) == res.shape[1], 't is unmatched {}/{}.'.format(len(frame_query), res.shape[1])
        return res
    ...
    def dynamic_mask_with_coords(self, mask_features, mask_head_params, reference_points, targets):
        ...
        if self.rel_coord:
            ...
            mask_features = repeat(mask_features, 'b t c h w -> b t q c h w', q=q) # [batch_size, time, num_queries_per_frame, c, h, w]
        mask_features = mask_features.reshape(1, -1, h, w)  # 1, btq c h w
