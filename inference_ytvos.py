'''
Inference code for ReferFormer, on refer_youtube_vos
...
import time
from util.misc import NestedTensor, nested_tensor_from_tensor_list, nested_tensor_from_videos_list
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for idx_, video in enumerate(video_list):
        ...
        # extract visual features once for all expressions to accelerate
        open_mem = True
        if open_mem:
            with torch.no_grad():
                with autocast(args.amp):
                    # Backbone
                    samples = nested_tensor_from_videos_list([imgs], 16)
                    features, visual_pos = model.backbone(samples)
                    input = None
                    memory_visual = [samples, features, visual_pos]
        else:
            input = [imgs]
            memory_visual = None
        ...
        for i in range(num_expressions):
            ...
            with torch.no_grad():
                with autocast(args.amp):
                    outputs = model(input, [exp], [target], memory_visual=memory_visual)
            ...
            frame_memory, _ = torch.sort(score_ind[:round(video_len * 0.25)])
            frame_query, _ = torch.sort(score_ind[round(video_len * 0.25):])
            vos_results = pred_masks
            if len(frame_query) > 0 and len(frame_memory) > 0:
                with torch.no_grad():
                    with autocast(args.amp):
                        ...
                        outputs = outputs[0][:, :, :img_h, :img_w]
                        outputs = F.interpolate(outputs, size=(origin_h, origin_w), mode='bilinear', align_corners=False)  # t 1 h w
                        outputs = torch.argmax(F.softmax(outputs, dim=1), dim=1)  # t h w
                        # put propagation results into previous rvos results.
                        vos_results[frame_query.cpu()] = outputs.detach().cpu().bool().numpy()
            else:
                print("None mem or query frame {}/{}".format(len(frame_memory), len(frame_query)))
            ...
            for j in range(video_len):
                frame_name = frames[j]
                mask = vos_results[j].astype(np.float32)
